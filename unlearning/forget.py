#!/usr/bin/env python3
"""Machine Unlearning — GA on forget (Sneha Singh), GD on retain (everyone else)."""
...
FORGET_NAME = "Sneha Singh"

def load_forget_retain(path="data/bio.jsonl"):
    forget, retain = [], []
...
def unlearn():
    forget_set, retain_set = load_forget_retain()
    ...
    # Repeat forget to match retain size
    ...
    for epoch in range(EPOCHS):
        ...
        # GRADIENT ASCENT on forget (push AWAY from Sneha Singh's bio)
        for batch in tqdm(forget_loader, desc=f"E{epoch+1} [GA-Forget]", leave=False):
            ...
            loss = -out.loss  # NEGATE = gradient ascent
            loss.backward(); ga_loss += out.loss.item()
            optimizer.step(); scheduler.step(); optimizer.zero_grad()

        # GRADIENT DESCENT on retain (preserve everyone else)
